...
void Stencils::VelocityBufferReadStencil::applyTopWall(FlowField& flowField, int i, int j) {
  if (parameters_.parallel.topNb >= 0) {
    if ((i >= 2 && i <= localSize[0] + 1)) {
      (flowField.getVelocity().getVector(i, j))[0] = *(topVelocityReadBuffer.get() + (i - 1));
      (flowField.getVelocity().getVector(i, j))[1] = *(topVelocityReadBuffer.get() + localSize[0] + (i - 1));
    } else if ((i == 1)) {
      (flowField.getVelocity().getVector(i, j))[0] = *(topVelocityReadBuffer.get() + (i - 1));
    }
  }
...
void Stencils::VelocityBufferReadStencil::applyBottomWall(FlowField& flowField, int i, int j) {
  if (parameters_.parallel.bottomNb >= 0) {
    if ((i >= 2 && i <= localSize[0] + 1)) {
      (flowField.getVelocity().getVector(i, j + 1))[0] = *(bottomVelocityReadBuffer.get() + (i - 1));
      (flowField.getVelocity().getVector(i, j))[1]     = *(bottomVelocityReadBuffer.get() + localSize[0] + (i - 1));
    } else if ((i == 1)) {
      (flowField.getVelocity().getVector(i, j + 1))[0] = *(bottomVelocityReadBuffer.get() + (i - 1));
    }
  }
  ...
  if (parameters_.parallel.leftNb >= 0) {
    if ((j >= 2) && (k >= 2) && j <= (localSize[1] + 1) && k <= (localSize[2] + 1)) {
      (flowField.getVelocity().getVector(i, j, k))[0] = *(
        leftVelocityReadBuffer.get() + (j - 2) + (k - 2) * localSize[1]
      );
      (flowField.getVelocity().getVector(i + 1, j, k))[1] = *(
        leftVelocityReadBuffer.get() + (localSize[1] * localSize[2]) + (j - 1) + ((k - 2) * (localSize[1]+1))
      );
      (flowField.getVelocity().getVector(i + 1, j, k))[2] = *(
        leftVelocityReadBuffer.get() + (localSize[1] * localSize[2]) + ((localSize[1] + 1) * localSize[2]) + (k-1)+((j-2)*(localSize[2]+1))
      );
...
void Stencils::VelocityBufferReadStencil::applyRightWall(FlowField& flowField, int i, int j, int k) {
  if (parameters_.parallel.rightNb >= 0) {
    if ((j >= 2) && (k >= 2) && j <= (localSize[1] + 1) && k <= (localSize[2] + 1)) {
      (flowField.getVelocity().getVector(i, j, k))[0] = *(
        rightVelocityReadBuffer.get() + (j - 2) + (k - 2) * localSize[1]
      );
      (flowField.getVelocity().getVector(i, j, k))[1] = *(
        rightVelocityReadBuffer.get() + (localSize[1] * localSize[2]) + (j - 1) + ((k - 2) * (localSize[1]+1))
      );
      (flowField.getVelocity().getVector(i, j, k))[2] = *(
        rightVelocityReadBuffer.get() + (localSize[1] * localSize[2]) + ((localSize[1] + 1) * localSize[2]) + (k-1)+((j-2)*(localSize[2]+1))
      );
    } else if ((j == 1) && (k >= 2) && k <= (localSize[2] + 1)) {
      (flowField.getVelocity().getVector(i, j, k))[1] = *(
        rightVelocityReadBuffer.get() + (localSize[1] * localSize[2]) + (j - 1) + ((k - 2) * (localSize[1]+1))
...
void Stencils::VelocityBufferReadStencil::applyTopWall(FlowField& flowField, int i, int j, int k) {
  if (parameters_.parallel.topNb >= 0) {
    ...
    if ((i >= 2) && (k >= 2) && i <= (localSize[0] + 1) && k <= (localSize[2] + 1)) {
      (flowField.getVelocity().getVector(i, j, k))[0] = *(
        topVelocityReadBuffer.get() + (i-1) + (k-2)*(localSize[0]+1)
      );
      (flowField.getVelocity().getVector(i, j, k))[1] = *(
        topVelocityReadBuffer.get() + ((localSize[0] + 1) * localSize[2]) + (i-2) + (k-2)*(localSize[0])
      );
      (flowField.getVelocity().getVector(i, j, k))[2] = *(
        topVelocityReadBuffer.get() + ((localSize[0] + 1) * localSize[2]) + (localSize[0] * localSize[2])
  ...
  if (parameters_.parallel.bottomNb >= 0) {
    if ((i >= 2) && (k >= 2) && i <= (localSize[0] + 1) && k <= (localSize[2] + 1)) {
      (flowField.getVelocity().getVector(i, j + 1, k))[0] = *(
        bottomVelocityReadBuffer.get() + (i-1) + (k-2)*(localSize[0]+1)
      );
      (flowField.getVelocity().getVector(i, j, k))[1] = *(
        bottomVelocityReadBuffer.get() + ((localSize[0] + 1) * localSize[2]) + (i-2) + (k-2)*(localSize[0])
      );
      (flowField.getVelocity().getVector(i, j + 1, k))[2] = *(
        bottomVelocityReadBuffer.get() + ((localSize[0] + 1) * localSize[2]) + (localSize[0] * localSize[2])
        + ((i - 2) * (localSize[2]+1)) + (k - 1)
      );
    }
  ...
  if (parameters_.parallel.frontNb >= 0) {
    if ((i >= 2) & (j >= 2) && i <= (localSize[0] + 1) && j <= (localSize[1] + 1)) {
      (flowField.getVelocity().getVector(i, j, k + 1))[0] = *(
        frontVelocityReadBuffer.get() + (i-1) + (j-2)*(localSize[0]+1)
      );
      (flowField.getVelocity().getVector(i, j, k + 1))[1] = *(
        frontVelocityReadBuffer.get() + ((localSize[0] + 1) * localSize[1]) + (j - 1) + ((i - 2) * (localSize[1]+1))
      );
      (flowField.getVelocity().getVector(i, j, k))[2] = *(
        frontVelocityReadBuffer.get() + (localSize[0] * (localSize[1] + 1)) + ((localSize[0] + 1) * localSize[1])
        + (j-2) + ((i-2) * localSize[1])
      );
...
void Stencils::VelocityBufferReadStencil::applyBackWall(FlowField& flowField, int i, int j, int k) {
  if (parameters_.parallel.backNb >= 0) {
    if ((i >= 2) & (j >= 2) && i <= (localSize[0] + 1) && j <= (localSize[1] + 1)) {
      (flowField.getVelocity().getVector(i, j, k))[0] = *(
        backVelocityReadBuffer.get() + (i-1) + (j-2)*(localSize[0]+1)
      );
      (flowField.getVelocity().getVector(i, j, k))[1] = *(
        backVelocityReadBuffer.get() + ((localSize[0] + 1) * localSize[1]) + (j - 1) + ((i - 2) * (localSize[1]+1))
      );
      (flowField.getVelocity().getVector(i, j, k))[2] = *(
        backVelocityReadBuffer.get() + (localSize[0] * (localSize[1] + 1)) + ((localSize[0] + 1) * localSize[1])
        + (j-2) + ((i-2) * localSize[1])
      );
    } else if ((i == 1) & (j >= 2) && j <= (localSize[1] + 1)) {
      (flowField.getVelocity().getVector(i, j, k))[0] = *(
